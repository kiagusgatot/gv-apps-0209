...
export default function BentoCard({
  ...
  if (variant === 'gradient' && gradient) {
    ...
    return (
      <Component
        onClick={onClick}
        className={`relative overflow-hidden rounded-2xl p-4 text-left transition-all duration-200 ${colSpanClass} ${rowSpanClass} ${
  ...
  const glassVariant =
    variant === 'dark' ? 'dark' : variant === 'white' ? 'elevated' : 'default'

  return (
    <GlassCard
      variant={glassVariant}
      onClick={onClick}
      className={`relative overflow-hidden p-4 ${colSpanClass} ${rowSpanClass} ${className}`}
      style={style}
      {...rest}
    >
      {children}
